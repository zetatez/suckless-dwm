...
# dwm - dynamic window manager (heavily patched fork)

dwm is an extremely fast, small, and dynamic window manager for X.

This is a **heavily patched fork** (version 6.8) maintained by [@zetatez](https://github.com/zetatez),
with **31 patches** applied adding scratchpads, vanity gaps, Fibonacci layouts, grid layouts,
terminal swallowing, per-tag settings, sticky windows, tag preview, actual fullscreen, and more.

## Patches

 | Patch                      | Description                                       |
 | -------                    | -------------                                     |
 | `actualfullscreen`         | Preserve window geometry when toggling fullscreen |
 | `activemonitor`            | Active monitor focus tracking                     |
 | `bar-height`               | Configurable bar height                           |
 | `barpadding`               | Bar padding support (`vertpad`, `sidepad`)        |
 | `bottomstack`              | Bottom stack layout                               |
 | `centerfirstwindow`        | Center first window on screen                     |
 | `columns`                  | Columns layout                                    |
 | `cyclelayouts`             | Cycle layouts forward/backward                    |
 | `deck-double-smartborders` | Deck layout + smart borders                       |
 | `fibonacci`                | Fibonacci spiral/dwindle layouts                  |
 | `focusmaster-return`       | Focus master window                               |
 | `gridmode`                 | Grid mode layout                                  |
 | `hide_vacant_tags`         | Hide empty tags from the bar                      |
 | `killunsel`                | Kill all unselected clients                       |
 | `leftstack`                | Left stack layout                                 |
 | `movestack`                | Move clients within the stack order               |
 | `movethrow`                | Move windows with keyboard shortcuts              |
 | `noborder`                 | Remove borders on single-window tags              |
 | `pertag`                   | Per-tag layout/mfact/nmaster settings             |
 | `restartsig`               | Restart dwm via SIGHUP signal                     |
 | `restoreafterrestart`      | Save/restore window tags across restart           |
 | `scratchpad`               | Scratchpad functionality (multiple instances)     |
 | `stairs`                   | Stairs layout                                     |
 | `sticky`                   | Sticky windows (visible on all tags)              |
 | `stickyindicator`          | Visual indicator for sticky windows               |
 | `swallow`                  | Terminal swallowing (GUI apps replace terminal)   |
 | `tag-preview`              | Preview another tag without switching             |
 | `tilewide`                 | Tile-wide layout variant                          |
...
## Layouts (13 total)
 ...
 | Key           | Symbol   | Layout                                                     |
 | -----         | -------- | --------                                                   |
 | `MOD+a`       | `[W]`    | Workflow (1-4 custom positioning, falls back to Fibonacci) |
 | `MOD+r`       | `[F]`    | Fibonacci spiral                                           |
 | `MOD+Shift+r` | `[F]`    | Fibonacci dwindle                                          |
 | `MOD+v`       | `[C]`    | Center free shape                                          |
 | `MOD+Shift+v` | `[C]`    | Center equal ratio                                         |
 | `MOD+t`       | `[T]`    | Tile right                                                 |
 | `MOD+Shift+t` | `[T]`    | Tile left                                                  |
 | `MOD+g`       | `[G]`    | Grid                                                       |
 | `MOD+Shift+g` | `[G]`    | Grid with gaps                                             |
 | `MOD+m`       | `[M]`    | Monocle                                                    |
 | `MOD+w`       | `[H]`    | Hacker (cascading offset)                                  |
 | `MOD+e`       | `[S]`    | Stack horizontal                                           |
 | `MOD+Shift+e` | `[S]`    | Stack vertical                                             |
 | `MOD+o`       | —        | Overview (expose mode)                                     |

## Requirements

```
libx11  libxft  libxinerama  imlib2
```

On Arch Linux: `sudo pacman -S libx11 libxft libxinerama imlib2`

## Installation

Edit `config.mk` to match your local setup (installed into `/usr/local` by default).
...
In order to display status info in the bar, pair with [dwmblocks](https://github.com/zetatez/suckless-dwmblocks).

## Configuration

Edit `config.h` and recompile. Three color themes are included and switchable at runtime via `SUP+t`.
...
### Alt Layer – Core Controls
 ...
 | `MOD+p`            | Launch the rofi application launcher             |
